    ...
    public partial class FreezableMemoryPoolStream
    {
        ...
        protected void SetLength(int value, in bool clear)
        {
            ...
            else
            {
                ...
                for (int len = buffers.Count; ; add -= lastBufferOffset)
                {
                    ...
                    if (clear) (lastBufferOffset == bufferLen ? buffer.Memory.Span : buffer.Memory.Span[..add]).Clear();
        ...
        protected ReadOnlySequence<byte> UpdateBufferSequence()
        {
            ...
            try
            {
                ...
                if (len == 0)
                {
                    BufferSequence = res = new(buffers[0].Memory[.._LastBufferOffset]);
                }
                else
                {
        ...
        private void ReturnBuffersOnDisposing()
        {
            ...
            {
                buffers.Frozen!.Value[1..].DisposeAll();
            }
            else
            {
                for (int i = 1, len = buffers.Count; i < len; buffers[i].Dispose(), i++) ;
            }
