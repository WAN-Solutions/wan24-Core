...
{
    // Extensions
    public partial class ImmutableArrayWhereEnumerable<T>
    {
        /// <summary>
        /// Where
        /// </summary>
        ...
        public virtual ImmutableArrayWhereEnumerable<T> Where(Func<T, bool> predicate) => new(Array, item => Predicate(item) && predicate(item), Offset, Length);
        ...
        /// <returns>Enumerable</returns>
        public virtual ImmutableArrayWhereSelectEnumerable<T, tResult> Select<tResult>(Func<T, tResult> selector) => new(Array, Predicate, selector, Offset, Length);
        ...
        /// <inheritdoc/>
        public virtual T FirstOrDefault(T defaultValue)
        {
            ImmutableArray<T> data = Array;
            for (int i = Offset, len = i + Length; i < len; i++)
                if (Predicate(data[i]))
        ...
        /// <returns>Enumerable</returns>
        public virtual ImmutableArrayWhereEnumerable<T> Skip(int count)
        {
            ImmutableArray<T> data = Array;
            for (int i = Offset, len = i + Length, cnt = 0; i < len; i++)
                if (Predicate(data[i]) && ++cnt > count)
                    return new(Array, Predicate, i, len - i);
        ...
        /// <param name="predicate">Predicate</param>
        /// <returns>Enumerable</returns>
        public virtual ImmutableArrayWhereEnumerable<T> SkipWhile(Func<T, bool> predicate)
        {
            ImmutableArray<T> data = Array;
            T item;
            for (int i = Offset, len = i + Length; i < len; i++)
        ...
        /// <param name="count">Count</param>
        /// <returns>Enumerable</returns>
        public virtual ImmutableArrayWhereEnumerable<T> Take(int count)
        {
            ImmutableArray<T> data = Array;
            for (int i = Offset, len = i + Length, cnt = 0; i < len; i++)
                if (Predicate(data[i]) && ++cnt >= count)
        ...
        /// <param name="predicate">Predicate</param>
        /// <returns>Enumerable</returns>
        public virtual ImmutableArrayWhereEnumerable<T> TakeWhile(Func<T, bool> predicate)
        {
            ImmutableArray<T> data = Array;
            T item;
            for (int i = Offset, len = i + Length; i < len; i++)
        ...
        public T[] ToArray()
        {
            using RentedArrayRefStruct<T> buffer = new(Length, clean: false);
