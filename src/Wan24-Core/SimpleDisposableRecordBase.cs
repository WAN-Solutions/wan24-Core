...
{
    /// <summary>
    /// Base class for a simple disposable record type without any extras and only a little overhead for comfort (no support for <see cref="DisposeAttribute"/>!)
    /// </summary>
    public abstract record class SimpleDisposableRecordBase : IDisposableObject
    {
        /// <summary>
        ...
        protected SimpleDisposableRecordBase(in bool allowFinalizer = false)
        {
            AllowFinalizer = allowFinalizer;
            if (CreateStackInfo) StackInfo = new StackInfo<SimpleDisposableRecordBase>(this);
        }
