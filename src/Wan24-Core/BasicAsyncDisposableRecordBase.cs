...
{
    /// <summary>
    /// Base class for an asynchronous disposable type (without any extras and only a little overhead for comfort (no support for <see cref="DisposeAttribute"/>!); without events)
    /// </summary>
    public abstract record class BasicAsyncDisposableRecordBase : IBasicDisposableObject, IAsyncDisposable
    {
        /// <summary>
        ...
        /// Destructor
        /// </summary>
        ~BasicAsyncDisposableRecordBase()
        {
        ...
        protected BasicAsyncDisposableRecordBase(in bool allowFinalizer = false)
        {
            AllowFinalizer = allowFinalizer;
            if (CreateStackInfo) StackInfo = new StackInfo<BasicAsyncDisposableRecordBase>(this);
        }
