...
namespace wan24.Core
{
    ...
    public sealed class ObjectLockManager<T> : SimpleDisposableBase, IObjectLockManager, IStatusProvider
    {
        /// <summary>
        /// Active locks
        /// </summary>
        ...
        public async Task<ObjectLock> LockAsync(object key, TimeSpan timeout, object? tag = null, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                while (EnsureUndisposed())
                {
                    ...
                    {
                        if (cancellationToken.IsEqualTo(default) || cancellationToken.IsEqualTo(ex.CancellationToken)) throw;
                    }
                    catch
                    {
                    }
