...
namespace wan24.Core.Enumerables
{
    ...
    public partial class ListWhereSelectEnumerable<tList, tItem, tResult>
    {
        ...
        public virtual ListWhereSelectEnumerable<tList, tItem, tResult2> Select<tResult2>(Func<tResult, tResult2> selector)
            => new(List, Predicate, item => selector(Selector(item)), Offset, Length);
        ...
        /// <returns>Enumerable</returns>
        public virtual ListWhereSelectEnumerable<tList, tItem, tResult> Skip(int count)
        {
            tList data = List;
            for (int i = Offset, len = i + Length, cnt = 0; i < len; i++)
                if (Predicate(data[i]) && ++cnt > count)
                    return new(List, Predicate, Selector, i, len - i);
            return new(CreateEmptyInstance(), Predicate, Selector);
        }
        ...
        /// <returns>Enumerable</returns>
        public virtual ListWhereSelectEnumerable<tList, tItem, tResult> SkipWhile(Func<tResult, bool> predicate)
        {
            tList data = List;
            tItem item;
            for (int i = Offset, len = i + Length; i < len; i++)
        ...
        /// <param name="count">Count</param>
        /// <returns>Enumerable</returns>
        public virtual ListWhereSelectEnumerable<tList, tItem, tResult> Take(int count)
        {
            tList data = List;
            for (int i = Offset, len = i + Length, cnt = 0; i < len; i++)
                if (Predicate(data[i]) && ++cnt >= count)
                    return new(List, Predicate, Selector, Offset, len - i);
            return new(CreateEmptyInstance(), Predicate, Selector);
        }
        ...
        /// <param name="predicate">Predicate</param>
        /// <returns>Enumerable</returns>
        public virtual ListWhereSelectEnumerable<tList, tItem, tResult> TakeWhile(Func<tResult, bool> predicate)
        {
            tList data = List;
            tItem item;
            for (int i = Offset, len = i + Length; i < len; i++)
