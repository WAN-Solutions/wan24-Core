...
namespace wan24.Core
{
    ...
    public static class PortKnocking
    {
        ...
        public static async Task CallTcpSynSequenceAsync(IPAddress target, TimeSpan timeout, ProcessingProgress? progress = null, CancellationToken cancellationToken = default, params int[] ports)
        {
        ...
        public static async Task CallUdpEmptySequenceAsync(IPAddress target, TimeSpan timeout, ProcessingProgress? progress = null, CancellationToken cancellationToken = default, params int[] ports)
        {
            if (ports.Length < 1 || ports.Any(p => p < 1 || p > ushort.MaxValue)) throw new ArgumentOutOfRangeException(nameof(ports));
