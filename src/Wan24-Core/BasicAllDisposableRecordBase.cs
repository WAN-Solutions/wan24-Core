...
{
    /// <summary>
    /// Base class for a disposable type (without any extras and only a little overhead for comfort (no support for <see cref="DisposeAttribute"/>!); without events)
    /// </summary>
    public abstract record class BasicAllDisposableRecordBase : IBasicDisposableObject, IDisposable, IAsyncDisposable
    {
        /// <summary>
        ...
        protected BasicAllDisposableRecordBase(in bool allowFinalizer = false)
        {
            AllowFinalizer = allowFinalizer;
            if (CreateStackInfo) StackInfo = new StackInfo<BasicAllDisposableRecordBase>(this);
        }
